...
export async function buildFallbackPipelineResult(
  ...
  if (allocation.totalOpportunityCost !== 0) {
    const label = allocation.totalOpportunityCost < 0 ? 'benefit from spending depreciating assets' : 'foregone yield'
    insightLines.push({
      kind: 'opportunity_cost',
      headline: 'Opportunity cost factored in',
      detail: `$${Math.abs(allocation.totalOpportunityCost).toFixed(3)}/mo ${label} was considered in the optimization.`,
  ...
  // Convert allocations to OptimizationResult format
